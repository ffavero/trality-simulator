...
from trality_simulator.portfolio import query_portfolio, query_balances
from trality_simulator.scheduler import schedule
from trality_simulator.orders import order_market_value
from trality_simulator.helpers import symbol_to_asset
from decimal import Decimal

# Actual bot code below this

"""
Multicoin Trality bot designed to:
  Buy crypto weekly for Dollar Cost Averaging

ZephyZephyr - 2021_0906
...
def init_symbol(
    ...
    buy_time - datetime object telling when to make each purchase
               (on or immediately after this datetime)
...
def handler(state, dataMap):
    ...
    for symbol, data in dataMap.items():
        ...
        # Get the current time. Trality time must be divided by 1000 to
        # get the datetime equivalent.
        ...
        if symbol_data["buy_time"] <= t_now:
            ...
            # Print balances for two periods:
            #   When orders are pending and when they have been filled.
